...
export const FestivalForm = ({ festivalId }: FestivalFormProps) => {
	const {
		register,
		handleSubmit,
		formState: { errors },
	...
	const onSubmit = async (data: any) => {
		try {
			// PUTリクエストでフェスティバル情報を更新
			const response = await axios.put(`/api/festival/${festivalId}`, data)
			alert('フェスティバル情報が更新されました')
		} catch (error) {
